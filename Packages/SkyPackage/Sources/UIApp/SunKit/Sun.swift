...
public struct Sun: Identifiable, Sendable {
    ...
    public var longitude: SunAngle {
        .init(degrees: location.coordinate.longitude)
    }
    
    /// Latitude of Location
    public var latitude: SunAngle {
    ...
    //Sun constants
    private let sunEclipticLongitudeAtTheEpoch: SunAngle = .init(degrees: 280.466069)
    private let sunEclipticLongitudePerigee:    SunAngle = .init(degrees: 282.938346)
    ...
    private func getSunMeanAnomaly(from elapsedDaysSinceStandardEpoch: Double) -> SunAngle {
        //Compute mean anomaly sun
        var sunMeanAnomaly: SunAngle  = .init(degrees:(((360.0 * elapsedDaysSinceStandardEpoch) / 365.242191) + sunEclipticLongitudeAtTheEpoch.degrees - sunEclipticLongitudePerigee.degrees))
        sunMeanAnomaly = .init(degrees: extendedMod(sunMeanAnomaly.degrees, 360))
        
        return sunMeanAnomaly
    }
    
    private func getSunEclipticLongitude(from sunMeanAnomaly: SunAngle) -> SunAngle {
        //eclipticLatitude
        ...
        var eclipticLatitude: SunAngle =  .init(degrees: trueAnomaly + sunEclipticLongitudePerigee.degrees)
    ...
    private mutating func updateSunCoordinates() {
        ...
        //Step10:
        var sunEclipticLongitude: SunAngle = .init(degrees: sunTrueAnomaly + sunEclipticLongitudePerigee.degrees)
    ...
    private func getSunrise() -> Date? {
        var haArg = (cos(SunAngle.degrees(90.833).radians)) / (cos(latitude.radians) * cos(sunEquatorialCoordinates.declination.radians)) - tan(latitude.radians) * tan(sunEquatorialCoordinates.declination.radians)
        
        haArg                 = clamp(lower: -1, upper: 1, number: haArg)
        let ha: SunAngle         = .radians(acos(haArg))
    ...
    private func getDateFrom(sunEvent : SunElevationEvents, morning: Bool = false) -> Date? {
        ...
        let elevationSun: SunAngle = .degrees(sunEvent.rawValue)
        ...
        let hraAngle: SunAngle = .radians(acos(cosHra))
